...
name(StoreID) ->
	list_to_atom("big_repack_" ++ big_storage_module:label_by_id(StoreID)).
            ...
            case big_entropy_gen:is_entropy_packing(Packing) of
                true ->
                    ...
                    RepackIOWorker = ?CHILD_WITH_ARGS(
                        big_repack_io, worker, big_repack_io:name(StoreID),
                        [big_repack_io:name(StoreID), StoreID]),
...
init({StoreID, ToPacking}) ->
	FromPacking = big_storage_module:get_packing(StoreID),
	?LOG_INFO([{event, big_repack_init},
        {name, name(StoreID)}, {store_id, StoreID},
		{from_packing, big_serialize:encode_packing(FromPacking, false)},
        {to_packing, big_serialize:encode_packing(ToPacking, false)}]),
	...
	PaddedModuleEnd = big_chunk_storage:get_chunk_bucket_end(ModuleEnd),
...
get_read_range(BucketEndOffset, ModuleStart, FootprintEnd, BatchSize) ->
	ReadRangeStart = big_chunk_storage:get_chunk_byte_from_bucket_end(BucketEndOffset),

	SectorSize = big_replica_2_9:get_sector_size(),
	ModuleStart2 = big_chunk_storage:get_chunk_bucket_start(ModuleStart + 1),
...
handle_cast(repack, #state{} = State) ->
	...
	State3 = case NewStatus of
		active ->
			repack(State2);
		paused ->
			big_util:cast_after(?DEVICE_LOCK_WAIT, self(), repack),
			State2;
		_ ->
			State2
	end,
...
handle_cast(count_states, #state{} = State) ->
	...
	big_util:cast_after(?STATE_COUNT_INTERVAL, self(), count_states),
	{noreply, State};
...
handle_info({entropy, BucketEndOffset, Entropies}, #state{} = State) ->
	...
	EntropyKeys = big_entropy_gen:generate_entropy_keys(RewardAddr, BucketEndOffset),
	EntropyOffsets = big_entropy_gen:entropy_offsets(BucketEndOffset),

	State2 = big_entropy_gen:map_entropies(
		Entropies, 
		EntropyOffsets,
		FootprintStart,
		FootprintEnd,
...
terminate(Reason, #state{} = State) ->
	log_debug(terminate, State, [{reason, big_util:safe_format(Reason)}]),
	store_cursor(State),
	ok.
	...
	case maps:size(Map) of
		0 ->
			...
			?LOG_INFO([{event, repacking_complete},
					{store_id, StoreID},
					{target_packing, big_serialize:encode_packing(TargetPacking, false)}]),
			State2;
		_ ->
			log_debug(repacking_complete_but_waiting, State, [
				{target_packing, big_serialize:encode_packing(TargetPacking, false)}]),
			big_util:cast_after(5000, self(), repack),
			State
	end;
	...
	case big_packing_server:is_buffer_full() of
		true ->
			log_debug(waiting_for_repack_buffer, State, [
				{target_packing, big_serialize:encode_packing(TargetPacking, false)}]),
			big_util:cast_after(200, self(), repack),
			State;
		false ->
...
repack_footprint(Cursor, #state{} = State) ->
	...
	BucketStartOffset = big_chunk_storage:get_chunk_bucket_start(Cursor),
	PaddedEndOffset = big_block:get_chunk_padded_offset(Cursor),

	%% sanity checks
	BucketEndOffset = big_chunk_storage:get_chunk_bucket_end(PaddedEndOffset),
	%% end sanity checks
	
	FootprintStart = BucketStartOffset+1,
	
	IsChunkRecorded = big_sync_record:is_recorded(PaddedEndOffset, ar_data_sync, StoreID),
	...
	case Skip orelse FootprintStart < ModuleStart orelse FootprintStart > ModuleEnd of
		...
		_ ->
			...
			State2 = State#state{ 
				footprint_start = FootprintStart,
				footprint_end = FootprintEnd,
				entropy_end = big_chunk_storage:get_chunk_bucket_end(EntropyEnd),
			...
			big_repack_io:read_footprint(FootprintOffsets, FootprintStart, FootprintEnd, StoreID),
...
footprint_end(FootprintOffsets, ModuleStart, ModuleEnd, BatchSize) ->
	...
	%% big_entropy_gen:footprint_end makes sure all offsets are in the same entropy partition
	FootprintEnd = big_entropy_gen:footprint_end(FirstOffset, ModuleEnd),
	min(big_chunk_storage:get_chunk_bucket_end(LastOffsetRangeEnd), FootprintEnd).
...
generate_repack_entropy(BucketEndOffset, #state{} = State) ->
	...
	big_entropy_gen:generate_entropies(StoreID, RewardAddr, BucketEndOffset, self()).
...
add_range_to_repack_chunk_map(OffsetChunkMap, OffsetMetadataMap, #state{} = State) ->
	...
	maps:fold(
		fun(AbsoluteEndOffset, Metadata, Acc) ->
			...
			BucketEndOffset = big_chunk_storage:get_chunk_bucket_end(AbsoluteEndOffset),
			PaddedEndOffset = big_block:get_chunk_padded_offset(AbsoluteEndOffset),

			IsRecorded = big_sync_record:is_recorded(PaddedEndOffset, ar_data_sync, StoreID),
...
maybe_repack_next_footprint(#state{} = State) ->
	...
	case maps:size(Map) of
		0 ->
			...
			big_repack_io:write_queue(WriteQueue, TargetPacking, RewardAddr, StoreID),
...
process_state_change(RepackChunk, #state{} = State) ->
	...
	case RepackChunk#repack_chunk.state of
		...
		needs_repack ->
			...
			big_packing_server:request_repack({BucketEndOffset, FootprintStart}, self(),
		...
		needs_encipher ->
			...
			big_packing_server:request_encipher(
